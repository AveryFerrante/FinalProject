...
int Stemmer2::vowelinstem()
{  int i; for (i = k0; i <= j; i++) if (! cons(i)) return TRUE;
   return FALSE;
}

int Stemmer2::doublec(int j)
{
...
void Stemmer2::setto(char * s)
{  int length = s[0];
   memmove(b+j+1,s+1,length);
   k = j+length;
}

void Stemmer2::r(char * s)
{
    if (m() > 0)
        setto(s);
}

void Stemmer2::step1ab()
{  if (b[k] == 's')
...
int Stemmer2::stem(char * p, int i, int j)
{
